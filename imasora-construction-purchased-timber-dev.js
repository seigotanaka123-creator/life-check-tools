...
import {hook,pickOption,craneBoardOption,craneExitOption} from './assets/imasora-construction-crane.js';
import {emptyPurchasedTimber,initialTimberPractice,purchasedTimberAction as actCrane,advancePurchasedTimber as stepCrane,packPurchasedTimber as packCrane,floatReadout,timberPhysicsState,FLOOR,TIMBER_MATERIALS,RETURN_PAD,returnOption,issueOption,timberCopyTotals} from './assets/imasora-construction-purchased-timber.js?v=455';
const TIMBER_PARTS=[FLOOR];
import {createCrane,updateCraneModel} from './assets/imasora-construction-crane-model.js';
import {createFloatingPart as createPart,animateFloatingPart} from './assets/imasora-construction-floating-model.js?v=447';
import {floatingPlacement as basePlacement} from './assets/imasora-construction-floating-physics.js';
import {PurchasedTimberSession,purchasedSourceReader} from './assets/imasora-construction-purchased-timber-storage.js?v=455';
...
const $=id=>document.getElementById(id),local=['localhost','127.0.0.1','[::1]'].includes(location.hostname),testMode=new URLSearchParams(location.search).get('timberPractice')==='1';
const placement=s=>basePlacement(timberPhysicsState(s));
let state=testMode?initialTimberPractice():emptyPurchasedTimber(),ready=false,roof=false,view=0,orbit=-2.6,pitch=.75,distance=285,time=0,walkPhase=0,dirtySince=0,loading=false,headContact='',headContactUntil=0,precision=false;
const session=testMode?{blocked:false,busy:false,savedRevision:-1}:new PurchasedTimberSession(),held=new Map(),keys=new Set(),scene=new THREE.Scene();scene.background=new THREE.Color(0xc6d8d2);scene.fog=new THREE.Fog(0xc6d8d2,800,1800);
...
function syncParts(){const ids=new Set(state.parts.map(p=>p.id));for(const [id,m]of partMeshes)if(!ids.has(id)){scene.remove(m);m.traverse(o=>{o.geometry?.dispose();if(o.material){for(const mat of [o.material].flat())mat.dispose();}});partMeshes.delete(id);}for(const p of state.parts)if(!partMeshes.has(p.id)){const m=createPart(p);partMeshes.set(p.id,m);scene.add(m);}}
...
function action(name){if(!ready||loading||session.blocked)return;clearInput();try{state=actCrane(state,name,crypto.randomUUID());}catch(e){state.message=e.message;}refresh();}
...
function refreshStock(){
  const t=timberCopyTotals(state);
  $('material-ledger').replaceChildren();
  const row=document.createElement('tr');for(const text of['火星木材',...['stored','loose','held','floating','fixed','total'].map(k=>t[k]+'枚')]){const cell=document.createElement('td');cell.textContent=text;row.append(cell);}$('material-ledger').append(row);
  $('scope').textContent=testMode?'貸出2枚 · 保存しない操作体験':'建設開発 8-3b2 · 購入済み木材の複製';
  $('source-info').textContent=testMode?'これは操作体験用の貸出床2枚です。金貨・購入品・既存の作業保存へ入りません。':state.source.mode==='copy'?`購入元：${state.source.kind} ／ 元の金貨 ${state.source.coins}枚 ／ 購入木材 ${state.source.total}枚分（原本は変更しません）。`:'購入済みの素材を一度だけ複製してください。木材0の保存なら0枚のままです。';
  $('imports').hidden=testMode||state.source.mode==='copy';
  $('reload').disabled=testMode||loading||session.busy;
  $('issue-floor').disabled=!ready||loading||session.blocked||!issueOption(state).ok;
  $('issue-floor').title=issueOption(state).reason;
  $('issue-hint').textContent=issueOption(state).reason;
  $('stock-count').textContent=`保管 ${state.stock}枚 ／ 作業場 ${state.parts.length}枚（同時8枚まで）。1枚の材積26.88。`;
}
$('issue-floor').onclick=()=>{action('issue');$('stock-message').textContent=state.message;refreshStock();};
$('stock-open').onclick=()=>{clearInput();refreshStock();$('stock-dialog').showModal();};
$('stock-close').onclick=()=>$('stock-dialog').close();
async function importStock(kind){if(testMode||loading)return;clearInput();loading=true;refreshStock();try{state=await session.importSource(purchasedSourceReader(kind),kind);$('stock-message').textContent='購入元を変更せず複製しました。';$('save-status').textContent='複製を保存済み';}catch(e){$('stock-message').textContent=e.message;}finally{loading=false;refreshStock();refresh();}}
$('import-copy').onclick=()=>importStock('v451-copy');
$('import-world').onclick=()=>importStock('v452-integration');
$('reload').onclick=async()=>{if(testMode||loading)return;clearInput();loading=true;try{const saved=await session.load();state=saved||emptyPurchasedTimber();$('stock-message').textContent=saved?'保存を読み直しました。一時停止を解除して再開できます。':'未複製です。';}catch(e){$('stock-message').textContent=e.message;}finally{loading=false;refreshStock();refresh();}};
$('return-stock').onclick=()=>action('return-stock');
$('export').onclick=()=>{try{$('export-data').textContent=JSON.stringify(packCrane(state),null,2);$('export-data').hidden=false;}catch(e){state.message=e.message;refresh();}};
async function save(){clearInput();if(testMode){$('save-status').textContent='操作確認用：保存しません';return true;}if(!ready||loading||session.busy||session.blocked||state.rig.transition)return false;let saved=false;try{await session.save(state);$('save-status').textContent='購入済み木材の作業コピーを保存しました';dirtySince=0;saved=true;}catch(e){$('save-status').textContent='保存停止：'+e.message;state.message=e.message;}refresh();return saved;}
$('save').onclick=save;
for(const link of document.querySelectorAll('[data-example]'))link.onclick=async e=>{
  ...
  if(!testMode&&state.source.mode==='copy'&&!await save())return;
...
function refresh(){
  const aboard=state.rig.mode==='driving',transition=!!state.rig.transition,work=state.work,p=state.parts.find(p=>p.id===state.held),candidate=pickOption(state),preview=placement(state),blocked=!ready||session.blocked||loading||state.paused;
  $('mode').textContent=state.paused?'一時停止':aboard?(work?'クレーン作業':'運転中'):transition?'乗り降り中':'徒歩';$('message').textContent=state.paused?'「作業を再開」で動き出します。':state.rig.mode==='foot'?(time<headContactUntil?`頭上に接触：${headContact}`:state.rig.hit?`接触：${state.rig.hit}`:state.message||'低い浮遊床へ歩いて乗ると、レンの重さで沈みます。'):state.message||state.rig.message;
  $('cargo').textContent=p?`吊り荷：${p.name}`:candidate?`フック下：${candidate.name}`:'吊り荷なし';$('count').textContent=`置場 ${state.parts.filter(p=>!p.fixed&&!p.hover&&p.id!==state.held).length} ・吊り荷 ${p?1:0} ・浮遊 ${state.parts.filter(p=>p.hover).length} ・設置 ${state.parts.filter(p=>p.fixed).length} ・保管 ${state.stock}`;
  $('material-summary').textContent=`${testMode?'貸出':'購入木材'} 合計 ${state.source.total}枚 ／ 保管 ${state.stock}枚 ／ 作業場 ${state.parts.length}枚`; 
  const active=state.parts.find(p=>p.hover);const floating=floatReadout(state,active?.id);
  $('float-load').textContent=floating.active?`浮遊床：基準 ${floating.target.toFixed(1)} / 現在 ${floating.height.toFixed(1)}｜荷重 ${floating.load.toFixed(2)} / ${floating.capacity.toFixed(1)}`:'火星木材を吊り、浮遊を開始すると高さを覚えます。';
  $('float-warning').textContent=floating.active?(floating.contact?`${floating.contact}に接触：安全停止`:floating.overload?'荷重オーバー：ゆっくり降下します':floating.rider?'レンの重さで沈んでいます。降りると戻ります。':'浮遊を保持しています。'):'';
  $('float-warning').classList.toggle('overload',floating.overload);
  $('release').textContent=p?.materialId==='mars-timber'?'浮遊を開始（吊り索解除）':'吊り索を外す';
  const recovery=returnOption(state);$('return-stock').disabled=blocked||!recovery.ok;$('return-stock').title=recovery.reason;$('return-hint').hidden=!p;$('return-hint').textContent=recovery.reason;$('stock-open').disabled=!ready||loading||transition;
  ...
  $('placement').hidden=!p;$('placement').textContent=preview?.reason||'';$('placement').classList.toggle('bad',!preview?.ok);$('save').disabled=!ready||loading||session.blocked||session.busy||transition||testMode||state.source.mode!=='copy';
  $('pause').textContent=state.paused?'作業を再開':'一時停止';$('pause').disabled=!ready||loading||session.blocked||transition;
}
async function boot(){
  if(!local){state.message='ローカル開発専用です。';refresh();return;}
  try{if(!testMode){const saved=await session.load();if(saved)state=saved;}ready=true;$('save-status').textContent=testMode?'貸出2枚：保存しません':state.source.mode==='copy'?'保存を読込（一時停止中）':'購入品を未複製';}
  catch(e){state.message=e.message;$('save-status').textContent='保存を保護して停止';}
  refresh();refreshStock();if(!testMode&&state.source.mode==='empty')$('stock-dialog').showModal();
}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;if(document.hidden)return;time+=dt;
  ...
  syncParts();const selected=pickOption(state);for(const m of partMeshes.values())m.visible=false;for(const p of state.parts){const m=partMeshes.get(p.id);m.visible=true;animateFloatingPart(m,p,time);m.position.set(p.x,p.y,p.z);m.rotation.y=p.angle;m.userData.edges.visible=p.id===selected?.id||p.id===state.held;m.userData.edges.material.color.setHex(p.id===state.held?0xa6f8e1:0xffd86e);}
  ...
  ui+=dt;if(ui>.12){ui=0;refresh();if(ready&&!testMode&&state.source.mode==='copy'&&!session.blocked&&state.revision!==session.savedRevision){if(!dirtySince)dirtySince=time;if(!session.busy)$('save-status').textContent='変更あり（自動保存待ち）';if(time-dirtySince>15&&!held.size&&!keys.size&&!state.rig.transition&&Math.abs(state.rig.vehicle.speed)<.01)save();}}
}
window.addEventListener('error',e=>{ready=false;clearInput();state.message='表示エラー：'+e.message;refresh();});window.addEventListener('unhandledrejection',e=>{ready=false;clearInput();state.message='処理エラー：'+(e.reason?.message||e.reason);refresh();});
