...
import {initialTimber as initialCrane,actTimber as actCrane,stepTimber as stepCrane,packTimber as packCrane,floatReadout,floatingDemo,TIMBER_PARTS,TIMBER_MATERIALS,RETURN_PAD,returnOption,issueOption} from './assets/imasora-construction-floating-timber.js?v=447';
...
import {floatingPlacement as placement} from './assets/imasora-construction-floating-physics.js';
import {TimberSession as CraneSession} from './assets/imasora-construction-floating-timber-storage.js?v=447';
...
const $=id=>document.getElementById(id),local=['localhost','127.0.0.1','[::1]'].includes(location.hostname),testMode=new URLSearchParams(location.search).get('floatingTest')==='1';
const dropDemo=testMode&&new URLSearchParams(location.search).get('floatingDemo')==='drop';
...
async function save(){clearInput();if(testMode){$('save-status').textContent='操作確認用：保存しません';return true;}if(!ready||loading||session.busy||session.blocked||state.rig.transition)return false;let saved=false;try{await session.save(state);$('save-status').textContent='保存済み（火星木材の開発専用）';dirtySince=0;saved=true;}catch(e){$('save-status').textContent='保存停止：'+e.message;state.message=e.message;}refresh();return saved;}
$('save').onclick=save;
...
function refresh(){
  ...
  $('mode').textContent=aboard?(work?'クレーン作業':'運転中'):transition?'乗り降り中':'徒歩';$('message').textContent=state.rig.mode==='foot'?(time<headContactUntil?`頭上に接触：${headContact}`:state.rig.hit?`接触：${state.rig.hit}`:dropDemo?'紫の火星木材は空中へ。茶色の普通の木材は地面へ。':'低い浮遊床へ歩いて乗ると、レンの重さで沈みます。'):state.message||state.rig.message;
  ...
  const floating=floatReadout(state);
...
async function boot(){
  ...
  const demo=new URLSearchParams(location.search).get('floatingDemo');
  if(testMode&&demo){state=floatingDemo(demo);orbit=Math.PI;pitch=.7;distance=230;}
  if(dropDemo){pitch=.65;distance=350;$('drop-demo').showModal();}
  try{if(!testMode){const saved=await session.load();if(saved)state=saved;}ready=true;$('save-status').textContent=testMode?'操作確認用：保存しません':session.savedRevision<0?'新しい火星木材の開発データ':'保存を読み込みました';}catch(e){state.message=e.message;$('save-status').textContent='保存を保護して停止';}refresh();
}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;if(document.hidden)return;time+=dt;
  ...
  if(dropDemo&&state.rig.mode==='foot'&&Math.hypot(pose.x,pose.z-65)<170)target.set(0,12,65);
