...
import {initialCrane,actCrane,stepCrane,hook,placement,pickOption,craneBoardOption,craneExitOption,packCrane,validateCrane} from './assets/imasora-construction-crane.js?v=438';
import {createCrane,updateCraneModel,createPart} from './assets/imasora-construction-crane-model.js?v=436';
import {CraneSession} from './assets/imasora-construction-crane-storage.js?v=436';
...
const $=id=>document.getElementById(id),local=['localhost','127.0.0.1','[::1]'].includes(location.hostname),testMode=new URLSearchParams(location.search).get('craneTest')==='1';
...
const partMeshes=new Map();for(const p of state.parts){const m=createPart(p);partMeshes.set(p.id,m);scene.add(m);}
...
async function save(){clearInput();if(testMode){$('save-status').textContent='操作確認用：保存しません';return true;}if(!ready||loading||session.busy||session.blocked||state.rig.transition)return false;let saved=false;try{await session.save(state);$('save-status').textContent='保存済み（クレーン専用）';dirtySince=0;saved=true;}catch(e){$('save-status').textContent='保存停止：'+e.message;state.message=e.message;}refresh();return saved;}
$('save').onclick=save;
...
window.addEventListener('keydown',e=>{if($('help').open||!ready)return;if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyE'].includes(e.code)){e.preventDefault();keys.add(e.code);if(e.code==='KeyE'&&!e.repeat)action('interact');}});
...
function refresh(){
  ...
  $('cargo').textContent=p?`吊り荷：${p.name}`:candidate?`フック下：${candidate.name}`:'吊り荷なし';$('count').textContent=`部材 ${state.parts.length} / 7 ・設置済み ${state.parts.filter(p=>p.fixed).length}`;
...
async function boot(){
  ...
  // A copy of the reported jam, only in the existing no-save test mode.
  // Never import this fixture into the player's persistent session.
  if(testMode&&new URLSearchParams(location.search).get('craneContactTest')==='1'){
    try{const res=await fetch('./backups/crane-freeze-before-v434/reported-state.json');if(!res.ok)throw new Error('再現データを読み込めません');state=validateCrane(await res.json());}
    catch(e){state.message=e.message;refresh();return;}
  }
  const example=new URLSearchParams(location.search).get('craneAssemblyTest');
  if(testMode&&example){
    try{const {craneAssemblyExample}=await import('./assets/imasora-construction-crane-examples.js?v=436');state=craneAssemblyExample(example,{finishWithCrane:new URLSearchParams(location.search).get('craneFinishTest')==='1'});orbit=example==='bridge'?-Math.PI/2:Math.PI;pitch=.3;distance=200;}
    catch(e){state.message=e.message;refresh();return;}
  }
  try{if(!testMode){const saved=await session.load();if(saved)state=saved;}ready=true;$('save-status').textContent=testMode?'操作確認用：保存しません':session.savedRevision<0?'新しいクレーン開発データ':'保存を読み込みました';}catch(e){state.message=e.message;$('save-status').textContent='保存を保護して停止';}refresh();
}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;if(document.hidden)return;time+=dt;
  ...
  const selected=pickOption(state);for(const p of state.parts){const m=partMeshes.get(p.id);m.position.set(p.x,p.y,p.z);m.rotation.y=p.angle;m.userData.edges.visible=p.id===selected?.id||p.id===state.held;m.userData.edges.material.color.setHex(p.id===state.held?0xa6f8e1:0xffd86e);}
