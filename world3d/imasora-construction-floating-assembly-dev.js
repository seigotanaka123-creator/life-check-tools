...
import {initialTimber as initialCrane,actTimber as actCrane,stepTimber as stepCrane,packTimber as packCrane,floatReadout,floatingDemo,joinOption,detachOption,component,TIMBER_PARTS,TIMBER_MATERIALS,RETURN_PAD,returnOption,issueOption} from './assets/imasora-construction-floating-assembly.js?v=448';
import {createCrane,updateCraneModel} from './assets/imasora-construction-crane-model.js';
import {createFloatingPart as createPart,animateFloatingPart} from './assets/imasora-construction-floating-model.js?v=448';
import {floatingPlacement as placement} from './assets/imasora-construction-floating-assembly-physics.js';
import {jointPose} from './assets/imasora-construction-floating-joints.js';
import {TimberSession as CraneSession} from './assets/imasora-construction-floating-assembly-storage.js?v=448';
...
const $=id=>document.getElementById(id),local=['localhost','127.0.0.1','[::1]'].includes(location.hostname),testMode=new URLSearchParams(location.search).get('assemblyTest')==='1';
const demoKind=testMode?new URLSearchParams(location.search).get('assemblyDemo'):null,dropDemo=['load','overload'].includes(demoKind);
...
// Small visible clamp strips span the actual contact seam; no separate floor collider.
const jointMeshes=Array.from({length:3},()=>{
  const g=new THREE.Group(),metal=mat(0xcba454,{metalness:.65,roughness:.35});
  for(const z of[-16,16]){
    const strip=new THREE.Mesh(new THREE.BoxGeometry(8,.28,3),metal);strip.position.set(0,0,z);g.add(strip);
    for(const x of[-2.6,2.6]){const bolt=new THREE.Mesh(new THREE.CylinderGeometry(.6,.6,.45,8),dark);bolt.position.set(x,.14,z);g.add(bolt);}
  }
...
for(const [id,name] of[['interact','interact'],['mode-toggle','mode'],['pick','pick'],['rotate','rotate'],['place','place'],['release','release'],['join','join'],['detach','detach'],['snap','snap'],['home','home']])$(id).onclick=()=>action(name);
...
$('drop-start').onclick=()=>{clearInput();$('drop-demo').close();};
...
function refreshStock(){
  ...
  for(const [id,row]of Object.entries(state.ledger)){
    ...
    for(const text of[TIMBER_MATERIALS[id],...['stored','loose','held','floating','fixed','total'].map(k=>unit(row[k]))]){const cell=document.createElement('td');cell.textContent=text;line.append(cell);}
  ...
  for(const p of TIMBER_PARTS){
    ...
    name.textContent=p.name;text.textContent=`材積 ${unit(p.w*p.h*p.d)} ｜ ${state.stock.includes(p.id)?'保管中':state.held===p.id?'吊り荷':state.parts.find(q=>q.id===p.id)?.hover?'浮遊中':state.parts.find(q=>q.id===p.id)?.fixed?'設置済み':'置場にあります'}`;
...
async function save(){clearInput();if(testMode){$('save-status').textContent='操作確認用：保存しません';return true;}if(!ready||loading||session.busy||session.blocked||state.rig.transition)return false;let saved=false;try{await session.save(state);$('save-status').textContent='保存済み（浮遊建築の開発専用）';dirtySince=0;saved=true;}catch(e){$('save-status').textContent='保存停止：'+e.message;state.message=e.message;}refresh();return saved;}
$('save').onclick=save;
...
window.addEventListener('keydown',e=>{if($('help').open||$('stock-dialog').open||$('drop-demo').open||!ready)return;if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyE'].includes(e.code)){e.preventDefault();keys.add(e.code);if(e.code==='KeyE'&&!e.repeat)action('interact');}});
...
function refresh(){
  ...
  $('mode').textContent=aboard?(work?'クレーン作業':'運転中'):transition?'乗り降り中':'徒歩';$('message').textContent=state.rig.mode==='foot'?(time<headContactUntil?`頭上に接触：${headContact}`:state.rig.hit?`接触：${state.rig.hit}`:dropDemo?'床の枚数によって、同じ重い鉄を支えられるか比べてください。':'接合した床の継ぎ目も、歩いて渡れます。'):state.message||state.rig.message;
  $('cargo').textContent=p?`吊り荷：${p.name}`:candidate?`フック下：${candidate.name}`:'吊り荷なし';$('count').textContent=`置場 ${state.parts.filter(p=>!p.fixed&&!p.hover&&p.id!==state.held).length} ・吊り荷 ${p?1:0} ・浮遊 ${state.parts.filter(p=>p.hover).length} ・設置 ${state.parts.filter(p=>p.fixed).length} ・保管 ${state.stock.length}`;
  $('material-summary').textContent=`貸出材積：火星木材 ${unit(state.ledger['mars-timber'].total)} ／ 普通木材 ${unit(state.ledger['earth-timber'].total)} ／ 鉄 ${unit(state.ledger['earth-iron'].total)}`;
  const joinPreview=joinOption(state),detach=detachOption(state),floating=floatReadout(state,candidate?.hover?candidate.id:undefined);
  $('join').disabled=blocked||!joinPreview.ok;$('join').title=joinPreview.reason;$('join').textContent=state.joining?`接合中 ${Math.round(state.joining.elapsed/1.2*100)}%`:'床に接合';
  $('detach').disabled=blocked||!detach.ok;$('detach').title=detach.reason;
  $('joint-count').textContent=`接合金具 ${state.joints.length}組${state.joining?' ｜ 面合わせ中':''}`;
  $('float-load').textContent=floating.active?`${floating.count}枚の浮遊床：基準 ${floating.target.toFixed(1)} / 現在 ${floating.height.toFixed(1)}｜荷重 ${floating.load.toFixed(2)} / ${floating.capacity.toFixed(1)}`:'火星木材を吊り、浮遊を開始すると高さを覚えます。';
  ...
  $('pick').disabled=blocked||!!p||!candidate||candidate?.hover&&component(state,candidate.id).length>1;$('pick').textContent=candidate?.fixed?'設置部材を吊り直す':'吊り索を掛ける';$('rotate').disabled=$('release').disabled=blocked||!p||!!state.rotation;$('place').disabled=blocked||!preview?.ok;
  ...
  $('snap').disabled=blocked||!!state.joining;
  ...
  $('placement-detail').textContent=state.joining?'接合完了まで待ってください。次の操作は予約しません。':joinPreview.ok?`接合予定 X ${joinPreview.candidate.x.toFixed(1)} / 高さ ${joinPreview.candidate.y.toFixed(1)} / Z ${joinPreview.candidate.z.toFixed(1)}`:cranePlacementReadout(state,preview);$('placement-detail').hidden=!p;
  for(const b of document.querySelectorAll('[data-action],#interact,#mode-toggle,#pick,#rotate,#place,#release,#return-stock,#snap,#precision,#join,#detach,#stock-open'))if(state.joining)b.disabled=true;
  $('placement').hidden=!p||!!state.joining;$('placement').textContent=joinPreview.ok?joinPreview.reason:preview?.reason||'';$('placement').classList.toggle('bad',!preview?.ok&&!joinPreview.ok);$('save').disabled=blocked||session.busy||transition||testMode;
}
async function boot(){
  if(!local){state.message='ローカル開発専用です。';refresh();return;}
  const demo=new URLSearchParams(location.search).get('assemblyDemo');
  if(testMode&&demo){state=floatingDemo(demo);orbit=Math.PI;pitch=.7;distance=demo==='join'?285:350;}
  if(dropDemo){pitch=.65;distance=350;$('drop-demo').showModal();}
  try{if(!testMode){const saved=await session.load();if(saved)state=saved;}ready=true;$('save-status').textContent=testMode?'操作確認用：保存しません':session.savedRevision<0?'新しい浮遊建築の開発データ':'保存を読み込みました';}catch(e){state.message=e.message;$('save-status').textContent='保存を保護して停止';}refresh();
}
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;if(document.hidden)return;time+=dt;
  ...
  const selected=pickOption(state);for(const m of partMeshes.values())m.visible=false;for(const p of state.parts){const m=partMeshes.get(p.id);m.visible=true;animateFloatingPart(m,p,time);m.position.set(p.x,p.y,p.z);m.rotation.y=p.angle;m.userData.edges.visible=p.id===selected?.id||p.id===state.held;m.userData.edges.material.color.setHex(p.id===state.held?0xa6f8e1:0xffd86e);}
  ...
  const joinPreview=joinOption(state),preview=state.joining?null:joinPreview.ok?joinPreview:placement(state);ghost.visible=!!preview;if(preview){const p=preview.candidate;ghost.position.set(p.x,p.y+p.h/2,p.z);ghost.rotation.y=p.angle;ghost.scale.set(p.w,p.h,p.d);ghost.material.color.setHex(preview.ok?0x5befad:0xeb9258);ghostEdges.material.color.setHex(preview.ok?0x74ffa9:0xffb18a);}
  jointMeshes.forEach(m=>m.visible=false);
  state.joints.forEach((j,i)=>{
    const a=state.parts.find(p=>p.id===j.a),b=state.parts.find(p=>p.id===j.b),m=jointMeshes[i],seam=jointPose(a,b);
    m.visible=true;m.position.set(seam.x,a.y+a.h+.12,seam.z);m.rotation.y=seam.angle;
  });
  ...
  if(dropDemo&&state.rig.mode==='foot'&&Math.hypot(pose.x,pose.z-65)<170)target.set(demoKind==='load'?40:0,12,65);
