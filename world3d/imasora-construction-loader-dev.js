...
function clearInput(){held.clear();keys.clear();document.querySelectorAll('.held').forEach(o=>o.classList.remove('held'));}
function interact(){clearInput();state=actLoader(state,'interact');refresh();}
$('interact').onclick=interact;$('home').onclick=()=>{clearInput();state=actLoader(state,'home');refresh();};
...
for(const b of document.querySelectorAll('[data-action]')){
  b.addEventListener('pointerdown',e=>{e.preventDefault();b.setPointerCapture(e.pointerId);held.set(e.pointerId,b.dataset.action);b.classList.add('held');});
...
window.addEventListener('keydown',e=>{if(!$('help').hidden)return;if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyE'].includes(e.code)){e.preventDefault();keys.add(e.code);if(e.code==='KeyE'&&!e.repeat)interact();}});
...
function controls(){
  ...
  if(state.mode==='driving')return loaderDrivingInput(held.values(),keys);
...
function frame(now){
  ...
  if(local&&$('help').hidden){accumulator+=dt;while(accumulator>=1/120){const before=actorPose(state),was=state.mode;state=stepLoader(state,controls(),1/120);const after=actorPose(state);if(state.mode!=='driving')walkPhase+=Math.hypot(after.x-before.x,after.z-before.z)*.2;if(was!==state.mode){clearInput();refresh();}accumulator-=1/120;}}else accumulator=0;
  const foldSteps=state.mode==='driving'||(state.mode==='boarding'&&state.transition.elapsed/state.transition.duration>.75);
  stepAmount=approach(stepAmount,foldSteps?0:1,dt*3);updateLoaderModel(loader,state.vehicle,{steps:stepAmount,roofTransparent});
  ...
  const entry=localToWorld(state.vehicle,-58,-8);ring.position.set(entry.x,.2,entry.z);ring.visible=entryArrow.visible=state.mode==='foot';entryArrow.position.set(entry.x,9+Math.sin(time*2)*1.1,entry.z);entryArrow.rotation.z=Math.PI;
...
if(!local)state.message='ローカル開発専用の確認画面です。';refresh();requestAnimationFrame(frame);
