...
const SHARK_ROLE_INITIAL_DELAY = 30;
const SHARK_ROLE_TRAVEL_SECONDS = 9.6;
const SHARK_ROLE_ALTERNATING_SLOT_SECONDS = 30;
...
const ROLE_SIDE_OUT_LANE_OUTER_ABS_X = 0.84;
...
const ROLE_SIDE_OUT_CAPTURE_Y = ROLE_SLOT_Y + 0.16;
...
class ImasoraJackpotCoinPusherGame {
  ...
  updateSharkDangerWarning() {
    ...
    warning.textMaterial.opacity = state.textOpacity;
  ...
  createPachinkoBoard() {
    ...
    [-1, 1].forEach(side => {
      ...
      chucker.add(mouth);
      const mark = createCheckerMarkMesh("1", 0.18, 0.13, checkerMarkMaterial);
      mark.name = side < 0
        ? "icp-hakama-chucker-left-mark-one"
        : "icp-hakama-chucker-right-mark-one";
  ...
  updatePachinkoTokens(delta) {
    for (let index = this.pachinkoTokens.length - 1; index >= 0; index -= 1) {
      ...
      const guidedSideOutSlot = this.guideRoleSideOutPocketToken(token);
      ...
      const roleSlotCaptureY = guidedSideOutSlot >= 0
        ? ROLE_SIDE_OUT_CAPTURE_Y
        : ROLE_SLOT_Y + 0.08;
      if (token.phase === "role" && token.body.position.y <= roleSlotCaptureY) {
        const slot = guidedSideOutSlot >= 0
          ? guidedSideOutSlot
          : token.body.position.x < -0.28
            ? 0
            : token.body.position.x > 0.28
              ? 2
              : 1;
  ...
  hasPusherPlateTopContact(coin) {
    ...
    return (this.world.contacts || []).some(contact => {
      if (contact.bi === this.pusherBody && contact.bj === coin.body) {
        return contact.si === this.pusherPlateShape
          && contact.ni.y >= PUSHER_COIN_TOP_CONTACT_NORMAL_MIN_Y;
      }
      if (contact.bj === this.pusherBody && contact.bi === coin.body) {
        return contact.sj === this.pusherPlateShape
          && -contact.ni.y >= PUSHER_COIN_TOP_CONTACT_NORMAL_MIN_Y;
      }
      return false;
    });
