...
export function createWorldSoilController({service,practice=false,preview=false,state,scene,camera,character,shadow,canvas,snapshot,clearInput,onExit,onError,readout}){
  ...
  function owns(value){if(value){for(const el of document.querySelectorAll('.world-toolbar,.control-card')){inert.set(el,el.inert);el.inert=true;}}else{for(const[el,was]of inert)el.inert=was;inert.clear();}canvas.parentElement.classList.toggle('world-soil-operating',value);}
  ...
  async function action(a){if(a==='begin'){begin();return;}if(!active||busy||blocked)return;clear();issue='';
    if(a==='camera'){view=(view+1)%3;button(a).textContent=['視点：追従','視点：横から','視点：区画全体'][view];return;}
  ...
  addEventListener('keydown',e=>{if(!active)return;if(codes.includes(e.code)){e.preventDefault();e.stopImmediatePropagation();if(!keys.has(e.code)&&!busy&&!blocked)taps.set(e.code,work.work.time+.15);keys.add(e.code);}else if(e.code==='KeyE'){e.preventDefault();e.stopImmediatePropagation();if(!e.repeat)void action('interact');}},true);
  ...
  function input(){const actions=[...held.values()],pressed=[...keys];for(const[k,t]of taps)if(t>work.work.time)(codes.includes(k)?pressed:actions).push(k);
    ...
    const angle=view===2?Math.PI:work.work.loader.vehicle.heading+(view===1?Math.PI/2:Math.PI);return{x:r*Math.cos(angle)-f*Math.sin(angle),z:-r*Math.sin(angle)-f*Math.cos(angle),jump:pressed.includes('Space')};
  }
  function refresh(){
    ...
    button('interact').textContent=l.mode==='foot'?'運転席に乗る':l.mode==='driving'?'停車して降りる':'乗り降り中…';button('interact').disabled=off||!!w.task||!!l.transition||!(l.mode==='foot'?w.body.grounded&&l.player.y===0&&boardOption(l,solids):exitOption(l,solids));
  ...
  function render(dt){if(!root)return;updateWorldSoilModel(root,work.work,active);if(!active)return;
    ...
    const a=view===2?Math.PI+.15:l.vehicle.heading+(view===1?Math.PI/2:Math.PI),pitch=view===2?1.1:.65,dist=(view===2?720:250)*Math.max(1,.72/camera.aspect);
  ...
  return {get active(){return active;},get dirty(){return !!work&&work.revision!==expected;},install,save,leave,begin,
    ...
    update(dt){if(state.map!=='construction'||!root)return false;
      ...
      if(active&&!busy&&!blocked&&!document.hidden){try{const mode=work.work.loader.mode;work=advanceWorldSoil(work,input(),dt);if(work.work.loader.mode!==mode)clear();auto+=dt;}catch(e){fail(e);}}
