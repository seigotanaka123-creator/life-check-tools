...
export function createWorldTimberController({service,practice=false,preview=false,state,scene,camera,character,shadow,canvas,snapshot,clearInput,onExit,onError,readout}){
  ...
  panel.innerHTML=`<header><strong>火星木材クレーン</strong><span class="wt-mode"></span><button data-action="begin">木材の作業を始める</button></header>
  ...
  function owns(value){if(value){for(const el of document.querySelectorAll('.world-toolbar,.control-card')){inert.set(el,el.inert);el.inert=true;}}else{for(const[el,was]of inert)el.inert=was;inert.clear();}canvas.parentElement.classList.toggle('world-timber-operating',value);}
  ...
  async function action(a){if(a==='begin'||a==='join-practice'){begin(a==='join-practice');return;}if(!active||busy||blocked)return;clear();issue='';
    if(a==='camera'){view=(view+1)%3;button(a).textContent=['視点：作業','視点：横から','視点：区画全体'][view];return;}
  ...
  for(const b of panel.querySelectorAll('[data-hold]')){
    b.addEventListener('pointerdown',e=>{if(!active||busy||blocked)return;e.preventDefault();held.set(e.pointerId,b.dataset.hold);taps.set(b.dataset.hold,time+.1);b.setPointerCapture(e.pointerId);b.classList.add('held');});
    ...
    b.addEventListener('click',e=>{if(e.detail===0&&active&&!busy&&!blocked)taps.set(b.dataset.hold,time+.1);});
  }
  const codes=['KeyW','KeyS','KeyA','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'];
  addEventListener('keydown',e=>{if(!active)return;if(codes.includes(e.code)){e.preventDefault();e.stopImmediatePropagation();if(!keys.has(e.code)&&!busy&&!blocked)taps.set(e.code,time+.1);keys.add(e.code);}else if(e.code==='KeyE'){e.preventDefault();e.stopImmediatePropagation();if(!e.repeat)void action('interact');}},true);
  ...
  const yaw=()=>view===1?-Math.PI/2:view===2?Math.PI:-2.6;
  ...
  function refresh(){const close=!!root&&state.map==='construction'&&Math.abs(state.position.x-root.position.x)<380&&Math.abs(state.position.z-root.position.z)<320;panel.hidden=!close&&!active;canvas.parentElement.classList.toggle('world-timber-near',!panel.hidden);if(panel.hidden)return;
    ...
    button('interact').textContent=aboard?'停車して降りる':moving?'乗り降り中…':'運転席に乗る';button('interact').disabled=off||!(aboard?!w.work&&w.deployment===0&&craneExitOption(w):craneBoardOption(w));
  ...
  function render(dt){if(!root)return;updateWorldTimberModel(root,work.work,{active,time});if(!active)return;
    ...
    const a=yaw(),pitch=view===2?1.1:.75,dist=(view===2?710:285)*Math.max(1,.9/camera.aspect);camera.position.copy(target).addScaledVector(new THREE.Vector3(Math.sin(a)*Math.cos(pitch),Math.sin(pitch),Math.cos(a)*Math.cos(pitch)),dist);camera.lookAt(target);readout();
  }
  ...
  return{get active(){return active;},get dirty(){return !!work&&work.revision!==expected;},install,save,leave,begin,
    ...
    update(dt){if(state.map!=='construction'||!root)return false;time+=dt;
      ...
      if(!busy&&!blocked&&!document.hidden){try{
